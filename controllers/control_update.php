...
if (empty($_FILES["pro_photo"]["name"])) {
    $insert_str = str_replace("pro_photo = :format,", "", $insert_str);
} else {
    $img = $_FILES["pro_photo"]; // recupère le nom de l'image du formulaire
    $img_name=basename($img["name"]);
    $fichier_path = "../assets/img/" . $img_name;
    move_uploaded_file ($_FILES["pro_photo"]["tmp_name"] ,$fichier_path);
}

$insert = $bd->prepare($insert_str);
